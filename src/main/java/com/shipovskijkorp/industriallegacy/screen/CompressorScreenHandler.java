...
/**
 * Compressor screen handler (IC2-style layout).
 *
 * Slots:
...
public class CompressorScreenHandler extends ScreenHandler {
    ...
    public CompressorScreenHandler(int syncId, PlayerInventory playerInv, PacketByteBuf buf) {
    ...
    public CompressorScreenHandler(int syncId, PlayerInventory playerInv, Inventory inv) {
        super(ModScreenHandlers.COMPRESSOR, syncId);

        if (inv == null) {
            this.inventory = new SimpleInventory(CompressorBlockEntity.INV_SIZE);
            ...
            this.inventory = inv;
            if (inv instanceof CompressorBlockEntity be) {
        ...
        this.addSlot(new Slot(this.inventory, CompressorBlockEntity.SLOT_INPUT, 56, 17));
    ...
    private static Inventory getBlockEntityInventory(PlayerInventory playerInv, PacketByteBuf buf) {
        ...
        if (be instanceof CompressorBlockEntity cbe) return cbe;
        return null;
    }
    ...
    public net.minecraft.item.ItemStack quickMove(PlayerEntity player, int index) {
        ...
        final int machineSlots = CompressorBlockEntity.INV_SIZE;
        ...
        if (index < machineSlots) {
            ...
            // from player -> machine
            if (!this.insertItem(original, CompressorBlockEntity.SLOT_INPUT, CompressorBlockEntity.SLOT_INPUT + 1, false)
                    && !this.insertItem(original, CompressorBlockEntity.SLOT_DISCHARGE, CompressorBlockEntity.SLOT_DISCHARGE + 1, false)
                    && !this.insertItem(original, CompressorBlockEntity.SLOT_UPGRADE_0, CompressorBlockEntity.SLOT_UPGRADE_0 + CompressorBlockEntity.UPGRADE_SLOTS, false)) {
                return net.minecraft.item.ItemStack.EMPTY;
            }
        }
