...
public class MvTransformerScreenHandler extends ScreenHandler {
    public static final int PROP_COUNT = 3;

    public final BlockPos pos;
    private final PropertyDelegate properties;

    public MvTransformerScreenHandler(int syncId, PlayerInventory playerInventory, PacketByteBuf buf) {
        this(syncId, playerInventory, buf.readBlockPos());
    }

    private MvTransformerScreenHandler(int syncId, PlayerInventory playerInventory, BlockPos pos) {
        this(syncId, playerInventory, pos, getClientProperties(playerInventory.player.getWorld(), pos));
    }

    public MvTransformerScreenHandler(int syncId, PlayerInventory playerInventory, MvTransformerBlockEntity blockEntity) {
        this(syncId, playerInventory, blockEntity.getPos(), blockEntity.getGuiProperties());
    }

    private MvTransformerScreenHandler(int syncId, PlayerInventory playerInventory, BlockPos pos, PropertyDelegate properties) {
        super(ModScreenHandlers.MV_TRANSFORMER, syncId);
        this.pos = pos;
    ...
    private static PropertyDelegate getClientProperties(World world, BlockPos pos) {
        if (world.getBlockEntity(pos) instanceof MvTransformerBlockEntity transformer) {
    ...
    public boolean canUse(PlayerEntity player) {
        if (player.getWorld().getBlockEntity(pos) instanceof MvTransformerBlockEntity) {
