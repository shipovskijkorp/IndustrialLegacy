...
public class HvTransformerScreenHandler extends ScreenHandler {
    public static final int PROP_COUNT = 3;

    public final BlockPos pos;
    private final PropertyDelegate properties;

    public HvTransformerScreenHandler(int syncId, PlayerInventory playerInventory, PacketByteBuf buf) {
        this(syncId, playerInventory, buf.readBlockPos());
    }

    private HvTransformerScreenHandler(int syncId, PlayerInventory playerInventory, BlockPos pos) {
        this(syncId, playerInventory, pos, getClientProperties(playerInventory.player.getWorld(), pos));
    }

    public HvTransformerScreenHandler(int syncId, PlayerInventory playerInventory, HvTransformerBlockEntity blockEntity) {
        this(syncId, playerInventory, blockEntity.getPos(), blockEntity.getGuiProperties());
    }

    private HvTransformerScreenHandler(int syncId, PlayerInventory playerInventory, BlockPos pos, PropertyDelegate properties) {
        super(ModScreenHandlers.HV_TRANSFORMER, syncId);
        this.pos = pos;
    ...
    public boolean canUse(PlayerEntity player) {
        if (player.getWorld().getBlockEntity(pos) instanceof HvTransformerBlockEntity) {
