...
public class LithiumFuelRodItem extends Item implements com.shipovskijkorp.industriallegacy.reactor.api.IReactorComponent {
    private static final String NBT_PROGRESS = "il_reactor_progress";
    private final int duration;

    public LithiumFuelRodItem(Settings settings, int duration) {
    ...
    public boolean acceptUraniumPulse(ItemStack stack, IReactor reactor, ItemStack pulsingStack,
        ...
        if (heatRun) {
            int newProgress = getProgress(stack) + reactor.getHeat() / 3000;
            if (newProgress >= duration) {
                reactor.setItemAt(youX, youY, new ItemStack(ModItems.TRITIUM_FUEL_ROD));
            } else {
