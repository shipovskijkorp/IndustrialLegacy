 ...
 *
 * Mechanics (IC2 Exp 1.12.2):
 * - maxCharge 160000 EU, tier 3, transferLimit 500 EU/t
 * - right click toggles "active" (requires >= 16 EU to turn on)
 * - base damage 4, active damage 20 if canUse(400 EU)
 * - hit consumes 400 EU when active; if not enough energy -> auto-off
 * - block break consumes 80 EU when active
 * - passive drain while active:
 *   - in hotbar: 64 EU per 16 ticks
 *   - elsewhere: 16 EU per 64 ticks
 *
 * Fabric 1.20.1 notes:
 * - SwordItem already implements FabricItem; we override the stack-aware
 *   getAttributeModifiers(ItemStack, EquipmentSlot) without calling FabricItem.super
 *   (Java disallows Interface.super if not directly declared).
 */
public final class NanoSaberItem extends SwordItem implements IElectricItem {
    ...
    /**
     * Fabric stack-aware modifiers (SwordItem implements FabricItem).
     */
    @Override
    public Multimap<EntityAttribute, EntityAttributeModifier> getAttributeModifiers(ItemStack stack, EquipmentSlot slot) {
        if (slot != EquipmentSlot.MAINHAND) {
            // vanilla sword behaviour for offhand/armor slots
