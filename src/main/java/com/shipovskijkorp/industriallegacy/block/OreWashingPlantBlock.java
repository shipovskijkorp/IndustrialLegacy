...
public class OreWashingPlantBlock extends BlockWithEntity {
    public static final DirectionProperty FACING = HorizontalFacingBlock.FACING;
    public static final BooleanProperty LIT = AbstractFurnaceBlock.LIT;

    public OreWashingPlantBlock(Settings settings) {
        super(settings);
    ...
    @Nullable @Override public BlockEntity createBlockEntity(BlockPos pos, BlockState state) { return new OreWashingPlantBlockEntity(pos, state); }
    @Nullable @Override public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        return world.isClient ? null : checkType(type, ModBlockEntities.ORE_WASHING_PLANT, OreWashingPlantBlockEntity::tick);
    }
    @Override public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        ...
        if (be instanceof OreWashingPlantBlockEntity machine) player.openHandledScreen(machine);
    ...
    @Override public void onStateReplaced(BlockState state, World world, BlockPos pos, BlockState newState, boolean moved) {
        ...
        if (!world.isClient && state.getBlock() != newState.getBlock()) EuNetwork.invalidate(world, pos);
    }
}
