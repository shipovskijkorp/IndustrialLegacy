...
/**
 * CESU (МЭСН) storage block (IC2 Exp semantics).
 *
 * Output side = {@link #FACING}.
 */
public class CesuBlock extends BlockWithEntity implements BlockEntityProvider {
    public static final DirectionProperty FACING = Properties.FACING;

    public CesuBlock(Settings settings) {
        super(settings);
    ...
    public int getWeakRedstonePower(BlockState state, BlockView world, BlockPos pos, Direction direction) {
        ...
        if (be instanceof CesuBlockEntity cesu) {
            return cesu.getRedstoneOutputLevel();
        }
        return 0;
    }
    ...
    public <T extends BlockEntity> @Nullable BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        ...
        return checkType(type, ModBlockEntities.CESU, CesuBlockEntity::tick);
    }
