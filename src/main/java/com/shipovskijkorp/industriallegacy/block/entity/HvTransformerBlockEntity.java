 ...
 */
public class HvTransformerBlockEntity extends BlockEntity implements IEuEnergyStorage, ExtendedScreenHandlerFactory {
    private static final int DEFAULT_TIER = 3; // LV
    ...
    private final PropertyDelegate guiProps = new PropertyDelegate() {
        @Override
        public int size() {
            return HvTransformerScreenHandler.PROP_COUNT;
        }
    ...
    public HvTransformerBlockEntity(BlockPos pos, BlockState state) {
        super(ModBlockEntities.HV_TRANSFORMER, pos, state);
    }

    public static void tick(World world, BlockPos pos, BlockState state, HvTransformerBlockEntity be) {
    ...
    private void syncActiveState() {
        ...
        if (!state.contains(HvTransformerBlock.ACTIVE)) return;

        boolean shouldBeActive = isStepUp();
        if (state.get(HvTransformerBlock.ACTIVE) != shouldBeActive) {
            world.setBlockState(pos, state.with(HvTransformerBlock.ACTIVE, shouldBeActive), Block.NOTIFY_ALL);
        }
    }

    private void invalidateNetwork() {
        if (world == null || world.isClient) return;
        HvTransformerBlock.invalidateAround(world, pos);
    }

    private Direction getFacing() {
        BlockState state = getCachedState();
        if (state.contains(HvTransformerBlock.DOT)) {
            return state.get(HvTransformerBlock.DOT);
        }
    ...
    public Text getDisplayName() {
        return Text.translatable("block.industrial_legacy.hv_transformer");
    }
    ...
    public ScreenHandler createMenu(int syncId, PlayerInventory inventory, PlayerEntity player) {
        return new HvTransformerScreenHandler(syncId, inventory, this);
    }
}
