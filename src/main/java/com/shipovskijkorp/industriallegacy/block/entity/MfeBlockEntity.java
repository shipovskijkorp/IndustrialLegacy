...
/**
 * IC2 Exp-style MFE (tier 3, 512 EU/t output, 4 000 000 EU capacity).
 *
...
public class MfeBlockEntity extends BlockEntity implements SidedInventory, IEuEnergyStorage, ExtendedScreenHandlerFactory {
    ...
    private final long capacity = 4000000L;
    private final int outputEuT = 512;
    ...
    private final PropertyDelegate guiProps = new PropertyDelegate() {
        ...
        public int get(int index) {
            return switch (index) {
                case 0 -> (int) Math.min(Integer.MAX_VALUE, MfeBlockEntity.this.energy);
                case 1 -> (int) Math.min(Integer.MAX_VALUE, MfeBlockEntity.this.capacity);
                case 2 -> MfeBlockEntity.this.outputEuT;
                case 3 -> MfeBlockEntity.this.redstoneMode;
                default -> 0;
            };
        }
        ...
        public void set(int index, int value) {
            switch (index) {
                case 0 -> MfeBlockEntity.this.energy = Math.min(MfeBlockEntity.this.capacity, Math.max(0L, value));
                case 3 -> MfeBlockEntity.this.redstoneMode = (byte) Math.max(0, Math.min(REDSTONE_MODES - 1, value));
                default -> {
                }
            }
        }
        ...
        @Override
        public int size() {
            return MfeScreenHandler.PROP_COUNT;
        }
    };

    public MfeBlockEntity(BlockPos pos, BlockState state) {
        super(ModBlockEntities.MFE, pos, state);
    }

    public static void tick(World world, BlockPos pos, BlockState state, MfeBlockEntity be) {
    ...
    private void emit() {
        ...
        Direction out = getCachedState().get(MfeBlock.FACING);
        if (out == null) return;
...
public ScreenHandler createMenu(int syncId, PlayerInventory inv, PlayerEntity player) {
    return new com.shipovskijkorp.industriallegacy.screen.MfeScreenHandler(syncId, inv, this);
}
}