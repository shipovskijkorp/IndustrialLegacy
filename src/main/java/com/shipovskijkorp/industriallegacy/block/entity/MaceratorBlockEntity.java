...
public class MaceratorBlockEntity extends BlockEntity implements SidedInventory, IEuEnergyStorage, ExtendedScreenHandlerFactory {
    ...
    public static void tick(World world, BlockPos pos, BlockState state, MaceratorBlockEntity be) {
        ...
        boolean lit = state.get(MaceratorBlock.LIT);
        if (lit != didWork) {
            world.setBlockState(pos, state.with(MaceratorBlock.LIT, didWork), 3);
        }
    ...
    private boolean processTick(World world) {
        MaceratorRecipe recipe = findRecipe(world).orElse(null);
        if (recipe == null) {
     ...
     */
    private Optional<MaceratorRecipe> findRecipe(World world) {
        Optional<?> opt = world.getRecipeManager().getFirstMatch(ModRecipes.MACERATOR_TYPE, this, world);
        if (opt.isEmpty()) return Optional.empty();

        Object o = opt.get();
        if (o instanceof MaceratorRecipe r) return Optional.of(r);

        // Try RecipeEntry#value() via reflection (if present).
        try {
            Method m = o.getClass().getMethod("value");
            Object v = m.invoke(o);
            if (v instanceof MaceratorRecipe r) return Optional.of(r);
    ...
    public Text getDisplayName() {
        return Text.translatable("container.industrial_legacy.macerator");
    }
    ...
    public ScreenHandler createMenu(int syncId, PlayerInventory inv, PlayerEntity player) {
        return new MaceratorScreenHandler(syncId, inv, this, props);
    }
}
