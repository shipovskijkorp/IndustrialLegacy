 ...
 */
public class CesuBlockEntity extends BlockEntity implements SidedInventory, IEuEnergyStorage, ExtendedScreenHandlerFactory, RedstoneModeCycleTarget {
    ...
    private final long capacity = 300000L;
    private final int outputEuT = 128;
    ...
    private final PropertyDelegate guiProps = new PropertyDelegate() {
        ...
        public int get(int index) {
            return switch (index) {
                case 0 -> (int) Math.min(Integer.MAX_VALUE, CesuBlockEntity.this.energy);
                case 1 -> (int) Math.min(Integer.MAX_VALUE, CesuBlockEntity.this.capacity);
                case 2 -> CesuBlockEntity.this.outputEuT;
                case 3 -> CesuBlockEntity.this.redstoneMode;
                default -> 0;
            };
        }
        ...
        public void set(int index, int value) {
            switch (index) {
                case 0 -> CesuBlockEntity.this.energy = Math.min(CesuBlockEntity.this.capacity, Math.max(0L, value));
                case 3 -> CesuBlockEntity.this.redstoneMode = (byte) Math.max(0, Math.min(REDSTONE_MODES - 1, value));
                default -> {
                }
            }
        }
        ...
        @Override
        public int size() {
            return CesuScreenHandler.PROP_COUNT;
        }
    };

    public CesuBlockEntity(BlockPos pos, BlockState state) {
        super(ModBlockEntities.CESU, pos, state);
    }
    ...
    public static void tick(World world, BlockPos pos, BlockState state, CesuBlockEntity be) {
    ...
    private void emit() {
        ...
        Direction out = getCachedState().get(CesuBlock.FACING);
        if (out == null) return;
...
public ScreenHandler createMenu(int syncId, PlayerInventory inv, PlayerEntity player) {
    return new com.shipovskijkorp.industriallegacy.screen.CesuScreenHandler(syncId, inv, this);
}
}