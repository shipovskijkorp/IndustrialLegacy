...
import com.shipovskijkorp.industriallegacy.screen.MvTransformerScreenHandler;
 ...
 */
public class MvTransformerBlockEntity extends BlockEntity implements IEuEnergyStorage, ExtendedScreenHandlerFactory {
    ...
    private final PropertyDelegate guiProps = new PropertyDelegate() {
        @Override
        public int size() {
            return MvTransformerScreenHandler.PROP_COUNT;
        }
    ...
    public MvTransformerBlockEntity(BlockPos pos, BlockState state) {
        super(ModBlockEntities.MV_TRANSFORMER, pos, state);
    }

    public static void tick(World world, BlockPos pos, BlockState state, MvTransformerBlockEntity be) {
    ...
    private void syncActiveState() {
        ...
        if (!state.contains(MvTransformerBlock.ACTIVE)) return;

        boolean shouldBeActive = isStepUp();
        if (state.get(MvTransformerBlock.ACTIVE) != shouldBeActive) {
            world.setBlockState(pos, state.with(MvTransformerBlock.ACTIVE, shouldBeActive), Block.NOTIFY_ALL);
        }
    }

    private void invalidateNetwork() {
        if (world == null || world.isClient) return;
        MvTransformerBlock.invalidateAround(world, pos);
    }

    private Direction getFacing() {
        BlockState state = getCachedState();
        if (state.contains(MvTransformerBlock.DOT)) {
            return state.get(MvTransformerBlock.DOT);
        }
    ...
    public Text getDisplayName() {
        return Text.translatable("block.industrial_legacy.mv_transformer");
    }
    ...
    public ScreenHandler createMenu(int syncId, PlayerInventory inventory, PlayerEntity player) {
        return new MvTransformerScreenHandler(syncId, inventory, this);
    }
}
